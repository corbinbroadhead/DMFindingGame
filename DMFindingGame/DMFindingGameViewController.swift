...
class DMFindingGameViewController: UIViewController {
    ...
    func newRound() {
        ...
        randomLetters = generateRandomLetters(numLetters: letterButtons.count)
    ...
    func generateRandomLetters(numLetters: Int) -> [String] {
        var initialArray = letters.shuffled()
        
        let numToRemove = letters.count - numLetters
        
        initialArray.removeLast(numToRemove)
        
        let target = targetLetter
        
        if initialArray.contains(target) == false {
            initialArray.removeLast()
            initialArray.append(target)
            initialArray.shuffle()
        }
